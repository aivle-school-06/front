...
2. **Git System (버전 관리):** 브랜치 이름은 \*\*영어(English)\*\*로, 커밋 메시지는 \*\*한국어(Korean)\*\*로 작성한다.
...
⚠️ Git 커밋 메시지는 반드시 한국어(Korean)로 작성한다.  
(응답, 설명, 문서는 한국어 / 커밋 메시지도 한국어)
...
* feat(auth): jwt 토큰 검증 필터 추가
* fix(order): 계산 로직 널 포인터 오류 수정
* docs(readme): 설치 가이드 업데이트
* refactor: 사용자 가입 흐름 단순화
* chore(deps): spring-boot 3.1.0 → 3.2.0 업데이트

#### **Incorrect (Bad)**

* feat: add login filter (English used)
